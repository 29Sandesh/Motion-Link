...
const GameScene: React.FC<GameSceneProps> = ({ 
  ...
  const vecA = useMemo(() => new Vector3(), []);
  const vecB = useMemo(() => new Vector3(), []);
  ...
  useFrame((state, delta) => {
    ...
    if (shakeIntensity.current > 0 && cameraRef.current) {
        ...
        // Decay shake
        shakeIntensity.current = MathUtils.lerp(shakeIntensity.current, 0, 10 * delta);
  ...
  const leftHandPosRef = useRef<Vector3 | null>(null);
  const rightHandPosRef = useRef<Vector3 | null>(null);
  const leftHandVelRef = useRef<Vector3 | null>(null);
  const rightHandVelRef = useRef<Vector3 | null>(null);
