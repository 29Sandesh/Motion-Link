...
interface SaberProps {
  type: HandType;
  positionRef: React.MutableRefObject<Vector3 | null>;
  velocityRef: React.MutableRefObject<Vector3 | null>;
}
...
const Saber: React.FC<SaberProps> = ({ type, positionRef, velocityRef }) => {
  ...
  useFrame((state, delta) => {
    ...
    if (targetPos) {
      ...
      meshRef.current.rotation.x = MathUtils.lerp(meshRef.current.rotation.x, targetRotation.current.x, 0.2);
      meshRef.current.rotation.y = MathUtils.lerp(meshRef.current.rotation.y, targetRotation.current.y, 0.2);
      meshRef.current.rotation.z = MathUtils.lerp(meshRef.current.rotation.z, targetRotation.current.z, 0.2);
